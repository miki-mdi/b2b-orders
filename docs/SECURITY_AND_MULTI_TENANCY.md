...
- Database-backed sessions, not stateless-only JWT: lets us immediately invalidate all sessions for a user (deactivated), a specific membership (revoked), or an entire tenant (suspended) by revoking rows, rather than waiting for token expiry.
